...
TABLE_FONT_SIZE = 16          # row text size
HEADER_FONT_FAMILY = "Segoe UI"
HEADER_FONT_SIZE = 18         # header text size
ROW_HEIGHT = 52               # row pixel height

# Column widths scaled up to match bigger content
COL_SERIAL_WIDTH = 120
COL_TASK_WIDTH   = 620
COL_STATUS_WIDTH = 170
COL_ACTION_WIDTH = 260

# Optional: make top controls larger too
TOP_FONT = ("Segoe UI", 14)
...
title_label = ctk.CTkLabel(header, text="Task Management System", font=TITLE_FONT)
...
task_title = ctk.CTkLabel(
    ...
    text="Task List",
    font=ctk.CTkFont("Times New Roman", 20, "bold"),
    fg_color="transparent",
)
...
# ---- Larger fonts and row height here ----
style.configure(
    "Treeview",
    background=BG_DARK,
    fieldbackground=BG_DARK,
    foreground=FG_TEXT,
    rowheight=ROW_HEIGHT,                                # ⬅ increase row height
    borderwidth=0,
    font=(TABLE_FONT_FAMILY, TABLE_FONT_SIZE)            # ⬅ larger row font
)
style.configure(
    "Treeview.Heading",
    background=HDR_BG,
    foreground=HDR_FG,
    relief="flat",
    font=(HEADER_FONT_FAMILY, HEADER_FONT_SIZE, "bold")  # ⬅ larger header font
)
style.map(
    "Treeview",
...
# Define columns (widths & alignment)  ⬅ use wider widths
tree.column("serial", width=COL_SERIAL_WIDTH, anchor="w", stretch=False)
tree.column("task",   width=COL_TASK_WIDTH,   anchor="w", stretch=True)
tree.column("status", width=COL_STATUS_WIDTH, anchor="w", stretch=False)
tree.column("action", width=COL_ACTION_WIDTH, anchor="center", stretch=False)
...
# Bind mouse click to tree  (fix HTML-escaped binding)
...
def open_add_task_dialog():
    ...
    lbl = ctk.CTkLabel(frm, text="Task Name:", font=TOP_FONT)
    ...
    submit_btn = ctk.CTkButton(btns, text="Submit", width=120, command=on_submit, font=TOP_FONT)
    cancel_btn = ctk.CTkButton(btns, text="Cancel", width=120, fg_color="#555555", hover_color="#444444",
                               command=on_cancel, font=TOP_FONT)
...
# -----------------------------
btn_add = ctk.CTkButton(controls, text="Add New Task", command=open_add_task_dialog, width=170, font=TOP_FONT)
...
btn_remove = ctk.CTkButton(controls, text="Remove All Task", command=remove_all_task, width=170, font=TOP_FONT)
...
filter_dropdown = ctk.CTkOptionMenu(
    ...
    width=170,
    font=TOP_FONT,                 # larger font for control
    dropdown_font=TOP_FONT         # larger font for dropdown items
)
...
# Bind resize to 'main' container (fix HTML-escaped binding)
